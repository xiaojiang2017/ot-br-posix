...
// Declared in openthread/openthread-system.h (not on this target's include
// path); defined in the posix platform library the agent links against.
extern "C" const otRcpInterfaceMetrics *otSysGetRcpInterfaceMetrics(void);
extern "C" const otRadioSpinelMetrics *otSysGetRadioSpinelMetrics(void);
...
static const char *NetDataServiceName(const otServiceConfig &aConfig)
{
    ...
    // DNS/SRP service: 0x5c + seqnum is the anycast form and 0x5d the
    // unicast form (0x5d + address + port, or a short form holding the
    // address in the server data); both use more than one service-data
    // byte, so match the first byte only (network_data_service.hpp
    // kDnsSrpAnycastServiceNumber / kDnsSrpUnicastServiceNumber).
    if (aConfig.mServiceDataLength >= 1 && aConfig.mServiceData[0] == 0x5c)
    {
        return "SRP server (anycast)";
    }
...
static const struct blobmsg_policy setSrpUnicastPolicy[1] = {
...
static const struct blobmsg_policy meshDiagPolicy[MESHDIAG_MAX] = {
    ...
    [MESHDIAG_RLOC16]               = {.name = "rloc16",               .type = BLOBMSG_TYPE_INT32},
};
...
static const struct ubus_method otbrAgentMethods[] = {
    ...
    {"setsrpsrvunicast",  &UbusAgentExt::HandleSetSrpUnicast,      0, 0, setSrpUnicastPolicy, ARRAY_SIZE(setSrpUnicastPolicy)},
    {"getsrpsrvunicast",  &UbusAgentExt::HandleGetSrpUnicast,      0, 0, nullptr, 0},
    ...
    {"metrics",           &UbusAgentExt::HandleMetrics,           0, 0, nullptr, 0},
...
UbusAgentExt::UbusAgentExt(Ncp::RcpHost *aController, std::mutex *aMutex)
    ...
    , mMeshDiagTargetMode(false)
    , mMeshDiagTargetResponded(false)
...
void UbusAgentExt::Initialize(Ncp::RcpHost *aController, std::mutex *aMutex)
{
...
void UbusAgentExt::HandleStateChanged(otChangedFlags aFlags)
{
    ...
    // Reconcile the unicast SRP supplement entry with the current role / SRP
    // server state. Idempotent; safe to call on every state-change batch.
    UpdateSrpUnicastSupplement(instance);
}
...
bool UbusAgentExt::ReadUciConfig(UciThreadConfig &aConfig)
{
    ...
    if (secSrpServer != nullptr)
    {
        ...
        // Optional: unicast SRP supplement (dual-publish), defaults to enabled.
        // Set option unicast_enabled='0' to disable publishing the unicast
        // DNS/SRP service entry alongside the anycast one.
        const char *uniEnabled = uci_lookup_option_string(uciCtx, secSrpServer, "unicast_enabled");
        if (uniEnabled != nullptr && (!strcmp(uniEnabled, "0") || !strcmp(uniEnabled, "false")))
        {
            mSrpUnicastEnabled = false;
            otbrLogInfo("otbr-agent: UCI srp_server unicast supplement disabled");
        }
    }
...
// ================ SRP unicast supplement (dual-publish) ====================
// While the SRP server runs in anycast mode it listens on port 53 for any
// address on the Thread interface. Additionally publish a unicast DNS/SRP
// service entry pointing to this device's mesh-local EID at the same port 53,
// so clients can reach the server either through the shared anycast ALOC or
// through this device's unicast address. The single :53 socket already serves
// both. The entry is reconciled on role / network-data changes.

static constexpr uint16_t kSrpUnicastSupplementPort = 53;
static constexpr uint32_t kSrpEnterpriseNumber      = 44970;
static constexpr uint8_t  kSrpUnicastServiceNumber  = 0x5d; // DNS/SRP unicast
static constexpr uint8_t  kSrpUnicastServiceDataLen = 1 + OT_IP6_ADDRESS_SIZE + sizeof(uint16_t);

// Build the unicast service-data for this device: 0x5d || mleid(16) || port(2 BE).
static uint8_t BuildSrpUnicastServiceData(otInstance *aInstance, uint8_t *aOut)
{
    const otIp6Address *mleid = otThreadGetMeshLocalEid(aInstance);

    aOut[0] = kSrpUnicastServiceNumber;
    memcpy(&aOut[1], mleid, OT_IP6_ADDRESS_SIZE);
    aOut[1 + OT_IP6_ADDRESS_SIZE]     = (uint8_t)(kSrpUnicastSupplementPort >> 8);
    aOut[1 + OT_IP6_ADDRESS_SIZE + 1] = (uint8_t)(kSrpUnicastSupplementPort & 0xff);

    return kSrpUnicastServiceDataLen;
}

// Returns true when this device's unicast supplement entry is present in the
// network data (matches enterprise + 0x5d + own RLOC16 + own ML-EID).
static bool FindSrpUnicastEntry(otInstance *aInstance)
{
    otNetworkDataIterator iterator = OT_NETWORK_DATA_ITERATOR_INIT;
    otServiceConfig       config;
    const otIp6Address   *mleid = otThreadGetMeshLocalEid(aInstance);

    while (otServerGetNextService(aInstance, &iterator, &config) == OT_ERROR_NONE)
    {
        if (config.mEnterpriseNumber == kSrpEnterpriseNumber && config.mServiceDataLength == kSrpUnicastServiceDataLen &&
            config.mServiceData[0] == kSrpUnicastServiceNumber &&
            config.mServerConfig.mRloc16 == otThreadGetRloc16(aInstance) &&
            memcmp(&config.mServiceData[1], mleid, OT_IP6_ADDRESS_SIZE) == 0)
        {
            return true;
        }
    }
...
// Reconcile the unicast supplement entry with the desired state. Idempotent:
// only adds when absent, only removes when present, so the network-data change
// it causes does not recurse.
void UbusAgentExt::UpdateSrpUnicastSupplement(otInstance *aInstance)
{
    ...
    bool shouldPublish = mSrpUnicastEnabled && IsThreadAttachedRole(otThreadGetDeviceRole(aInstance)) &&
                         (otSrpServerGetState(aInstance) == OT_SRP_SERVER_STATE_RUNNING) &&
                         (otSrpServerGetAddressMode(aInstance) == OT_SRP_SERVER_ADDRESS_MODE_ANYCAST);
    bool present = FindSrpUnicastEntry(aInstance);

    if (shouldPublish && !present)
    {
        otServiceConfig config;
        memset(&config, 0, sizeof(config));
        config.mEnterpriseNumber     = kSrpEnterpriseNumber;
        config.mServiceDataLength    = BuildSrpUnicastServiceData(aInstance, config.mServiceData);
        config.mServerConfig.mStable = true;
        config.mServerConfig.mRloc16 = otThreadGetRloc16(aInstance);

        otError error = otServerAddService(aInstance, &config);
        if (error == OT_ERROR_NONE)
        {
            // otServerAddService only appends to the local network data; push
            // it to the leader so the entry is visible network-wide.
            error = otServerRegister(aInstance);
            if (error == OT_ERROR_NONE)
            {
                otbrLogInfo("otbr-agent: published unicast SRP supplement entry (ML-EID port %u)",
                            kSrpUnicastSupplementPort);
            }
        }
        if (error != OT_ERROR_NONE)
        {
            otbrLogWarning("otbr-agent: failed to publish unicast SRP supplement entry: %s",
                           otThreadErrorToString(error));
        }
    }
    else if (!shouldPublish && present)
    {
        uint8_t serviceData[kSrpUnicastServiceDataLen];
        uint8_t length = BuildSrpUnicastServiceData(aInstance, serviceData);

        otError error = otServerRemoveService(aInstance, kSrpEnterpriseNumber, serviceData, length);
        if (error == OT_ERROR_NONE)
        {
            // Push the removal to the leader as well (see above).
            error = otServerRegister(aInstance);
            if (error == OT_ERROR_NONE)
            {
                otbrLogInfo("otbr-agent: removed unicast SRP supplement entry");
            }
        }
        if (error != OT_ERROR_NONE)
        {
            otbrLogWarning("otbr-agent: failed to remove unicast SRP supplement entry: %s",
                           otThreadErrorToString(error));
        }
    }
}

// ================ Handler: setsrpsrvunicast ====================
// ubus call otbr-agent setsrpsrvunicast '{"enable":true}'
int UbusAgentExt::HandleSetSrpUnicast(struct ubus_context      *aContext,
                                      struct ubus_object       *aObj,
                                      struct ubus_request_data *aRequest,
                                      const char               *aMethod,
                                      struct blob_attr         *aMsg)
{
    OT_UNUSED_VARIABLE(aObj);
    OT_UNUSED_VARIABLE(aMethod);
    return GetInstance().SetSrpUnicastDetail(aContext, aRequest, aMsg);
}

int UbusAgentExt::SetSrpUnicastDetail(struct ubus_context      *aContext,
                                      struct ubus_request_data *aRequest,
                                      struct blob_attr         *aMsg)
{
    blob_buf_init(&mBuf, 0);

    struct blob_attr *tb[1] = {nullptr};
    if (aMsg != nullptr)
    {
        blobmsg_parse(setSrpUnicastPolicy, 1, tb, blob_data(aMsg), blob_len(aMsg));
    }

    mNcpThreadMutex->lock();
    if (tb[0] != nullptr)
    {
        mSrpUnicastEnabled = blobmsg_get_bool(tb[0]);
    }
    UpdateSrpUnicastSupplement(mController->GetInstance());
    mNcpThreadMutex->unlock();

    blobmsg_add_u8(&mBuf, "enable", mSrpUnicastEnabled ? 1 : 0);
    blobmsg_add_u8(&mBuf, "err_code", 0);
    ubus_send_reply(aContext, aRequest, mBuf.head);
    return 0;
}

// ================ Handler: getsrpsrvunicast ====================
// ubus call otbr-agent getsrpsrvunicast
int UbusAgentExt::HandleGetSrpUnicast(struct ubus_context      *aContext,
                                      struct ubus_object       *aObj,
                                      struct ubus_request_data *aRequest,
                                      const char               *aMethod,
                                      struct blob_attr         *aMsg)
{
    ...
    OT_UNUSED_VARIABLE(aMethod);
    OT_UNUSED_VARIABLE(aMsg);
    return GetInstance().GetSrpUnicastDetail(aContext, aRequest);
}

int UbusAgentExt::GetSrpUnicastDetail(struct ubus_context *aContext, struct ubus_request_data *aRequest)
{
    blob_buf_init(&mBuf, 0);
    mNcpThreadMutex->lock();
    bool published = FindSrpUnicastEntry(mController->GetInstance());
    mNcpThreadMutex->unlock();

    blobmsg_add_u8(&mBuf, "enable", mSrpUnicastEnabled ? 1 : 0);
    blobmsg_add_u8(&mBuf, "published", published ? 1 : 0);
    ubus_send_reply(aContext, aRequest, mBuf.head);
    return 0;
}
...
// Defined alongside the srpnetservice handler below; reused here so TXT
// output is binary-safe and identical in shape to srpnetservice replies.
    ...
    while ((host = otSrpServerGetNextHost(instance, host)) != nullptr)
    {
        ...
        while ((service = otSrpServerHostGetNextService(host, service)) != nullptr)
        {
            ...
            if (!isDeleted)
            {
                ...
                blobmsg_add_string(&mBuf, "ServiceName", otSrpServerServiceGetServiceName(service));
                ...
                if (txtDataLength > 0)
                {
                    ...
                    {
                        std::string item = FormatTxtEntry(entry);
                        blobmsg_add_string(&mBuf, nullptr, item.c_str());
                    }
...
// Resolve the server address of an SRP server netdata entry, used to tell
// whether the entry is the one the local SRP client is registered with.
// Anycast entries (service number 0x5c + seqnum) are reached at the service
// ALOC derived from the Service ID (mle_types.hpp ServiceAlocFromId:
// 0xfc10 + ServiceId). Unicast entries (service number 0x5d) encode the
// address either in the service data (preferred unicast: 0x5d + address +
// port) or in the server data (address + port); a server data holding only
// a port number means the server's RLOC is the address
// (network_data_service.cpp GetNextDnsSrpUnicastInfo).
static bool GetSrpServerAddress(otInstance *aInstance, const otServiceConfig &aConfig, otIp6Address &aAddress)
{
    const otMeshLocalPrefix *prefix = otThreadGetMeshLocalPrefix(aInstance);

    if (aConfig.mServiceDataLength < 1)
    {
        return false;
    }

    if (aConfig.mServiceData[0] == 0x5c)
    {
        // Service ALOC: mesh-local prefix + 0000:00ff:fe00:(0xfc10 + ServiceId)
        uint16_t aloc16 = static_cast<uint16_t>(0xfc10 + aConfig.mServiceId);
        memset(&aAddress, 0, sizeof(aAddress));
        memcpy(&aAddress, prefix->m8, OT_MESH_LOCAL_PREFIX_SIZE);
        aAddress.mFields.m8[11] = 0xff;
        aAddress.mFields.m8[12] = 0xfe;
        aAddress.mFields.m8[14] = static_cast<uint8_t>(aloc16 >> 8);
        aAddress.mFields.m8[15] = static_cast<uint8_t>(aloc16 & 0xff);
        return true;
    }

    if (aConfig.mServiceData[0] != 0x5d)
    {
        return false;
    }

    if (aConfig.mServiceDataLength >= 1 + sizeof(otIp6Address) + sizeof(uint16_t))
    {
        memcpy(&aAddress, &aConfig.mServiceData[1], sizeof(otIp6Address));
        return true;
    }

    if (aConfig.mServerConfig.mServerDataLength >= sizeof(otIp6Address) + sizeof(uint16_t))
    {
        memcpy(&aAddress, &aConfig.mServerConfig.mServerData[0], sizeof(otIp6Address));
        return true;
    }

    if (aConfig.mServerConfig.mServerDataLength == sizeof(uint16_t))
    {
        // RLOC: mesh-local prefix + 0000:00ff:fe00:<rloc16>
        memset(&aAddress, 0, sizeof(aAddress));
        memcpy(&aAddress, prefix->m8, OT_MESH_LOCAL_PREFIX_SIZE);
        aAddress.mFields.m8[11] = 0xff;
        aAddress.mFields.m8[12] = 0xfe;
        aAddress.mFields.m8[14] = static_cast<uint8_t>(aConfig.mServerConfig.mRloc16 >> 8);
        aAddress.mFields.m8[15] = static_cast<uint8_t>(aConfig.mServerConfig.mRloc16 & 0xff);
        return true;
    }
                                 ...
                                 struct ubus_request_data *aRequest)
{
    ...
    // The SRP client auto-start registers with a single SRP server (the
    // numerically lowest server address); its currently selected server
    // address identifies which published entry is actually registered to.
    const otSockAddr *registeredSrpServer = otSrpClientGetServerAddress(instance);
    ...
    while (otNetDataGetNextService(instance, &iterator, &config) == OT_ERROR_NONE)
    {
        ...
        // Identify service type; match the first service-data byte only.
        // SRP entries appear in two forms: anycast (0x5c + seqnum) and
        // unicast (0x5d, followed by address/port encodings longer than
        // the plain 1-byte form).
        if (config.mServiceDataLength == 1 && config.mServiceData[0] == 0x01)
        {
            blobmsg_add_string(&mBuf, "ServiceName", "BBR");
        }
        else if (config.mServiceDataLength >= 1 && (config.mServiceData[0] == 0x5c || config.mServiceData[0] == 0x5d))
        {
            otIp6Address serverAddress;
            bool        isRegistered = GetSrpServerAddress(instance, config, serverAddress) &&
                                !otIp6IsAddressUnspecified(&registeredSrpServer->mAddress) &&
                                otIp6IsAddressEqual(&serverAddress, &registeredSrpServer->mAddress);

            blobmsg_add_string(&mBuf, "ServiceName",
                               config.mServiceData[0] == 0x5c ? "SRP server (anycast)" : "SRP server");
            blobmsg_add_u8(&mBuf, "IsRegistered", isRegistered ? 1 : 0);
        }
        ...
        char rloc16Str[8] = {0};
        snprintf(rloc16Str, sizeof(rloc16Str), "0x%04x", config.mServerConfig.mRloc16);
        blobmsg_add_string(&mBuf, "Rloc16s", rloc16Str);
                                 ...
                                 struct blob_attr         *aMsg)
{
    ...
    // ---- Targeted mode: query a single router by RLOC16 ----
    // Skips topology discovery entirely (saves ~N over-the-air diagnostic transactions).
    // Basic router info is seeded from the local router table; only the requested
    // per-router query phases (childtable/childip6/routerneighbortable) run for the target.
    if (tb[MESHDIAG_RLOC16] != nullptr)
    {
        uint32_t targetRloc16 = blobmsg_get_u32(tb[MESHDIAG_RLOC16]);
        if (targetRloc16 > 0xffff)
        {
            blobmsg_add_string(&mBuf, "Error", "invalid_rloc16");
            blobmsg_add_u32(&mBuf, "ErrorCode", OT_ERROR_INVALID_ARGS);
            ubus_send_reply(aContext, aRequest, mBuf.head);
            return 0;
        }

        // Targeted mode defaults: childtable + childip6 on, neighbor table off
        bool tWantChildTable = (tb[MESHDIAG_CHILDTABLE] != nullptr) ? blobmsg_get_bool(tb[MESHDIAG_CHILDTABLE]) : true;
        bool tWantChildIp6   = (tb[MESHDIAG_CHILDIP6] != nullptr) ? blobmsg_get_bool(tb[MESHDIAG_CHILDIP6]) : true;
        bool tWantRouterNbr  = (tb[MESHDIAG_ROUTERNEIGHBORTABLE] != nullptr) ? blobmsg_get_bool(tb[MESHDIAG_ROUTERNEIGHBORTABLE]) : false;

        MeshDiagRouterResult result{};
        result.mRouterId = (uint8_t)(targetRloc16 >> 10);
        result.mRloc16   = (uint16_t)targetRloc16;
        result.mVersion  = OT_MESH_DIAG_VERSION_UNKNOWN;

        // Seed basic info from the local router table (no over-the-air traffic)
        mNcpThreadMutex->lock();
        otInstance *instance = mController->GetInstance();

        otRouterInfo routerInfo;
        if (otThreadGetRouterInfo(instance, result.mRouterId, &routerInfo) == OT_ERROR_NONE &&
            routerInfo.mRloc16 == result.mRloc16)
        {
            result.mExtAddress = routerInfo.mExtAddress;
            result.mVersion    = routerInfo.mVersion;
        }

        const otExtAddress *ownAddr = otLinkGetExtendedAddress(instance);
        if (memcmp(&result.mExtAddress, ownAddr, OT_EXT_ADDRESS_SIZE) == 0)
        {
            result.mIsThisDevice = true;
            result.mIsLeader     = (otThreadGetDeviceRole(instance) == OT_DEVICE_ROLE_LEADER);
        }

        {
            std::lock_guard<std::mutex> lock(mMeshDiagMutex);
            mMeshDiagResults.clear();
            mMeshDiagResults.push_back(result);
            mMeshDiagPhase              = kMeshDiagTopology;
            mMeshDiagRouterIdx          = 0;
            mMeshDiagDone               = false;
            mMeshDiagFinalError         = OT_ERROR_NONE;
            mMeshDiagTargetMode         = true;
            mMeshDiagTargetResponded    = false;
            mMeshDiagWantChildTable     = tWantChildTable;
            mMeshDiagWantChildIp6       = tWantChildIp6;
            mMeshDiagWantRouterNeighbor = tWantRouterNbr;
        }
        ...
        // Kick the state machine: the kMeshDiagTopology branch moves directly to the
        // first enabled per-router phase for the seeded target (index 0).
        AdvanceMeshDiagPhase();
        mNcpThreadMutex->unlock();

        otbrLogInfo("otbr-agent: meshdiag targeted query started for rloc16 0x%04x", (unsigned)targetRloc16);
        return 0;
    }
...
void UbusAgentExt::HandleMeshDiagChildTableResult(otError aError, const otMeshDiagChildEntry *aChildEntry)
{
    ...
    // A completed query (with or without entries) means the target router is reachable
    if (aChildEntry != nullptr || aError == OT_ERROR_NONE)
    {
        mMeshDiagTargetResponded = true;
    }
                                                ...
                                                otMeshDiagIp6AddrIterator *aIp6AddrIterator)
{
    ...
    // A completed query (with or without entries) means the target router is reachable
    if (aIp6AddrIterator != nullptr || aError == OT_ERROR_NONE)
    {
        mMeshDiagTargetResponded = true;
    }
                                                            ...
                                                            const otMeshDiagRouterNeighborEntry *aNeighborEntry)
{
    ...
    // A completed query (with or without entries) means the target router is reachable
    if (aNeighborEntry != nullptr || aError == OT_ERROR_NONE)
    {
        mMeshDiagTargetResponded = true;
    }
...
void UbusAgentExt::CompleteMeshDiagDeferred(void)
{
    ...
    // Total child count across all routers. A child's RLOC16 is derived from its
    // parent router's RLOC16, so children never collide across routers and the
    // per-router unique counts sum to the network-wide total.
    ...
    for (const auto &router : mMeshDiagResults)
    {
        ...
        // In targeted mode, report whether the target actually answered any query phase
        AddBlobMsgBool(&b, "IsOnline", mMeshDiagTargetMode ? mMeshDiagTargetResponded : true);
        ...
        totalChildCount += allChildRlocs.size();
    ...
    blobmsg_add_u32(&b, "ChildNumber", (uint32_t)totalChildCount);
...
// ================ Handler: metrics =============================
// ubus call otbr-agent metrics
// Returns RCP interface frame counters (host side of the UART link) and
// spinel metrics. Used to locate frame loss on the host<->RCP path.

int UbusAgentExt::HandleMetrics(struct ubus_context      *aContext,
                                struct ubus_object       *aObj,
                                struct ubus_request_data *aRequest,
                                const char               *aMethod,
                                struct blob_attr         *aMsg)
{
    ...
    OT_UNUSED_VARIABLE(aMethod);
    OT_UNUSED_VARIABLE(aMsg);
    return GetInstance().MetricsDetail(aContext, aRequest);
}

int UbusAgentExt::MetricsDetail(struct ubus_context      *aContext,
                                struct ubus_request_data *aRequest)
{
    blob_buf_init(&mBuf, 0);

    const otRcpInterfaceMetrics *im = otSysGetRcpInterfaceMetrics();
    if (im != nullptr)
    {
        blobmsg_add_u64(&mBuf, "rx_frames", im->mRxFrameCount);
        blobmsg_add_u64(&mBuf, "rx_bytes", im->mRxFrameByteCount);
        blobmsg_add_u64(&mBuf, "tx_frames", im->mTxFrameCount);
        blobmsg_add_u64(&mBuf, "tx_bytes", im->mTxFrameByteCount);
        blobmsg_add_u64(&mBuf, "transferred_frames", im->mTransferredFrameCount);
        blobmsg_add_u64(&mBuf, "valid_frames", im->mTransferredValidFrameCount);
        blobmsg_add_u64(&mBuf, "garbage_frames", im->mTransferredGarbageFrameCount);
    }

    const otRadioSpinelMetrics *sm = otSysGetRadioSpinelMetrics();
    if (sm != nullptr)
    {
        blobmsg_add_u32(&mBuf, "rcp_timeout_count", sm->mRcpTimeoutCount);
        blobmsg_add_u32(&mBuf, "rcp_unexpected_reset_count", sm->mRcpUnexpectedResetCount);
        blobmsg_add_u32(&mBuf, "rcp_restoration_count", sm->mRcpRestorationCount);
        blobmsg_add_u32(&mBuf, "spinel_parse_error_count", sm->mSpinelParseErrorCount);
    }
